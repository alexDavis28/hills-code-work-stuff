...
namespace Dastan
{
        ...
        private void CreateBoard()
        {
            ...
            for (int Row = 1; Row <= NoOfRows; Row++)
            {
                for (int Column = 1; Column <= NoOfColumns; Column++)
                {
                    if (Row == 1 && Column==1)
                    {
                        S = new Promotion(Players[1], "p");
                    }
                    else if (Row == 1 && Column == NoOfColumns / 2)
                    {
                        S = new Kotla(Players[0], "K");
                    }
                    else if (Row == 1 && Column == NoOfColumns)
                    {
                        S = new Promotion(Players[1], "p");
                    }
                    else if (Row==NoOfRows && Column==1)
                    {
                        S = new Promotion(Players[0], "P");
                    }
    ...
    {
        public Promotion(Player P, string S) : base()
        {
            BelongsTo = P;
            Symbol = S;
        }

        public override void GetPointsForOccupacy(Player CurrentPlayer)
        {
            if (PieceInSquare != null)
            {
                if (BelongsTo.SameAs(PieceInSquare.GetBelongsTo()))
                {
                    Piece P;
                    if (PieceInSquare.GetSymbol()=="!")
                    {
                        P = new Piece("mirza", BelongsTo, 5, "1");
                    }
                    else
                    {
                        P = new Piece("mirza", BelongsTo, 5, "2");
                    }
                    RemovePiece();
                    SetPiece(P);
                }
